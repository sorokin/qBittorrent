...
void QBtSession::processBigRatios() {
  ...
  for ( ; torrentIT != torrentITend; ++torrentIT) {
    ...
    if (h.is_seed()) {
      ...
      if (ratio_limit == TorrentPersistentData::instance().USE_GLOBAL_RATIO)
        ratio_limit = global_ratio_limit;
      if (ratio_limit == TorrentPersistentData::instance().NO_RATIO_LIMIT)
        continue;
...
void QBtSession::removeRatioPerTorrent(const QString &hash)
{
  ...
  TorrentPersistentData::instance().setRatioLimit(hash, TorrentPersistentData::instance().USE_GLOBAL_RATIO);
  updateRatioTimer();
}
